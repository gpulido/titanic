...
data <- read.csv('../data/train.csv',stringsAsFactors = FALSE, na.strings = "")
dim.data.frame(data)
...
knitr::kable(tabla.normalidad, booktabs = T) %>% 
...
```{r}
knitr::kable(tabla.modelos, booktabs = T) %>% 
  kable_styling(latex_options = c("striped"))
```
...
```{r, message= FALSE, warning=FALSE, echo=FALSE}
summary(model_tree2$finalModel)
```
...
fichero.nuevo <- "../final_data/titanic_final.csv"
...
fichero.nuevo <- "../final_data/titanic_predict.csv"
write.csv(data, file=fichero.nuevo, row.names = FALSE)
