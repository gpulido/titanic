...
precision <- function(fit_tree, data_test, test_y, titulo) {
    predict_unseen <- predict(fit_tree, data_test, type = 'class')
...
```{r warning=FALSE,  echo= FALSE}
tabla.modelos[nrow(tabla.modelos)+1,] <- precision(bc_tree, data_test, testy, "C50 1")
...
```{r warning=FALSE, echo= FALSE}
tabla.modelos[nrow(tabla.modelos)+1,]<- precision(bc_winno_tree, data_test, testy, "C50 2")
```
...
```{r warning=FALSE,  echo= FALSE}
tabla.modelos[nrow(tabla.modelos)+1,] <- precision(bc_boost_tree, data_test, testy, "C50 3")
...
Con respecta a los modelos predictivos, se han ido también creando gráficas representando las tablas de confusión y extrayendo la precisión de los modelos.
